...
import ReceiptFormalTemplate from "../../../components/Templates/reciept/ReceiptFormal.template";
import ReceiptInFormalTemplate from "../../../components/Templates/reciept/ReceiptInFormal.template";

function CustomInvoicePaidReceiptPage() {
  const { id } = useParams();
  ...
  const [informalStyling, setInformalStyling] = useState({
    background: `url(${BackgroundPattern1})`,
    color: "#20927c",
    cardBackground: "#0b2637",
  });
  const colorList = ["#000", "#fff", "transparent", "#E377C2", "#7F7F7F", "#BCBD22", "#17BECF"];
  const patternURLList = [
    `url(${BackgroundPattern1})`,
    `url(${BackgroundPattern2})`,
    `url(${BackgroundPattern3})`,
    `url(${BackgroundPattern4})`,
    `url(${BackgroundPattern5})`,
  ];
      ...
      <div className="w-full overflow-y-auto">
        <div>
          {invoiceType === "pdf" ? (
            ...
            <button className="form-button hover:underline" onClick={() => setInvoiceType("pdf")}>
              Business Invoice
            </button>
          )}
          ...
          {invoiceType === "pdf" ? (
            <div className="flex flex-col items-center ">
              {customInvoice.invoice_data ? (
                <PDFViewer className="w-full h-[900px]">
                  <ReceiptFormalTemplate invoice={customInvoice.invoice_data} payment_details={customInvoice.payment_details} />
                </PDFViewer>
              ) : (
                "Loading..."
              )}
            ...
            <div className="overflow-y-scroll flex flex-col items-center bg-[#2a2a2e]">
                ...
                <div className="group">
                  ...
                  <span className="absolute right-20 hidden group-focus-within:grid group-hover:grid grid-cols-3	gap-3 p-2  bg-[#2a2a2e] border rounded">
                    ...
                    <input
                      ...
                      onChange={(e) => setInformalStyling((state) => ({ ...state, background: e.target.value }))}
                    />
                  </span>
                </div>
                <div className="group">
                  ...
                  <span className="absolute right-20 hidden group-focus-within:grid group-hover:grid grid-cols-3	gap-3 p-2  bg-[#2a2a2e] border rounded">
                    ...
                    <input
                      type="color"
                      className="  h-[25px] w-[25px] rounded"
                      onChange={(e) => setInformalStyling((state) => ({ ...state, color: e.target.value }))}
                    />
                  </span>
                </div>
                <div className="group">
                  ...
                  <span className="absolute right-20 hidden group-focus-within:grid group-hover:grid grid-cols-3	gap-3 p-2  bg-[#2a2a2e] border rounded">
                    ...
                    <input
                      type="color"
                      className=" h-[25px] w-[25px] rounded"
                      onChange={(e) => setInformalStyling((state) => ({ ...state, cardBackground: e.target.value }))}
                    />
                  </span>
                </div>
                <button
              ...
              <div className="py-10">
                <ReceiptInFormalTemplate id="custom-invoice-html" invoice={customInvoice} styling={informalStyling} />
              </div>
            </div>
          )}
        </div>
...
export default CustomInvoicePaidReceiptPage;
